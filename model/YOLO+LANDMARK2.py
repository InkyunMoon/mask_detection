'''
(미해결)
0222 - terminal에서 argments입력시 오류 발생
...
ap.add_argument('-n', '--name', required=True, help='name of yolo model')
...
labelsPath = os.path.sep.join([args['yolo'], args['name'] + '.names'])
...
weightsPath = os.path.sep.join([args['yolo'], args['name'] + '.weights'])
configPath = os.path.sep.join([args['yolo'], args['name'] + '.cfg'])
...
predictor = dlib.shape_predictor(os.getcwd() + '/landmark/optimal_eye_predictor.dat')
    ...
    for out in outs:
        for detection in out:
            ...
            if confidence > 0.5:
                ...
                confidences.append([float(args['confidence'])])
                classIDs.append(classID)
